...
void test() {
  Timer timer;
  // Asynchronous start. std::async must immediately create a new thread and start the execution of the task in the new
  // thread
  std::future<size_t> evenSumFuture = std::async(std::launch::async, findEven, start, end);

  //  Deferred execution. When using this strategy, tasks are delayed until the caller accesses the get() or wait()
  //  method of the std::future object associated with std::async.
...
// DETACH NOTES
// 0. This is used to detach newly created thread from the parent thread. It means the parent thread or main thread
would not wait for its finish.
// 1. Always check before detaching a thread that it is joinable otherwise we may end up double detaching and
// double detach() will result into the wohle program termination.
// 2. If we have detached thread and main function is returning then the detached thread execution is suspended (Means
it still runs background, not terminated). But in parent thread we would not care it anymore.

// NOTES:
// Either join() or detach() should be called on thred object, otherwise during thread object's destructor it will
// terminate the program. Because inside destructor it checks if thread is still joinable? if yes then it terminates the
program.
*/
void run(int count) {
  while (count > 0) {
...
/*
// TOPIC: Mutex In C++ Threading | Why Use Mutex | What Is Race Condition And How To Solve It? | What Is Critical
Section
...
void taskWithDeferLock(const char* threadNumber, int loopFor) {
  std::unique_lock<std::mutex> lock(
    mtx_buffer, std::defer_lock);  // do not acquire ownership of the mutex yet, because used defer_lock
...
// 1. Condition Variable allows running threads to wait on some conditions and once those conditions are met the waiting
thread
//	  is notified using :
                a. notify_one();
                b. notify_all();
...
reacquired. The thread should then check the condition and resume waiting if the wake up was spurious.
...
void withdrawMoneyWithWaitFor(int numMoney) {
  ...
  if (cv_money.wait_for(lk, std::chrono::seconds(8), [numMoney]() {
        // In the case of a wait_for timeout, the condition is indeed checked one last time. This is to confirm that the
        // condition is satisfied at the moment of timeout, and to correctly reflect the state of the condition even if
        // it is woken up at exactly the moment of timeout.
        bool res = (balance > numMoney);
        if (!res) {
          std::cout << "Money withdraw failed. Balance: " << balance << ", withdraw: " << numMoney << std::endl;
        }
...
void withdrawMoneyWithWaitUntil(int numMoney) {
  ...
  if (cv_money.wait_until(lk, deadline, [numMoney]() {
    ...
    std::cout << "Timeout, Reached the deadline!"
              << " Withdraw money : " << numMoney << " failed!\n";
  }
}
...
void process(const std::string& workerName) {
  while (goldAmount < 100 || diamondAmount < 100) {
    ...
    std::cout << workerName << " digging and found a gold and diamond, Gold: " << ++goldAmount
              << ", Diamount: " << ++diamondAmount << std::endl;
...
void       processVerifyDeadlockMike() {
  while (goldAmount < 100 || diamondAmount < 100) {
    std::lock(mtx_gold, mtx_diamond);
    std::lock(mtx3, mtx4);
    std::cout << "Mike digging and found a gold and diamond, Gold: " << ++goldAmount
              << ", Diamount: " << ++diamondAmount << std::endl;
...
void processVerifyDeadlockJerry() {
  while (goldAmount < 100 || diamondAmount < 100) {
    std::lock(mtx3, mtx4);
    std::lock(mtx_gold, mtx_diamond);
    std::cout << "Jerry digging and found a gold and diamond, Gold: " << ++goldAmount
              << ", Diamount: " << ++diamondAmount << std::endl;
...
void test() {
  ...
  // Specifically, std::promise cannot be copied, only moved, and attempting to copy it (as the current code effectively
  // does when passing it to the thread) will lead to a compilation error. std::thread t1(findOdd, promise, start, end);
...
void producer(int amount) {
  while (amount > 0) {
    std::unique_lock lk(mtx_dq);
    cv.wait(lk, []() { return dq.size() < g_buffersize; });
...
For std::thread, if the thread is still joinable upon destruction, it leads to a call to std::terminate, potentially
causing errors in applications (not terminating all threads). Using std::jthread eliminates the need to explicitly join
threads and helps in avoiding issues related to incomplete thread termination.
*/
void process(int id) {
...
/*
   ...
   leaves the scope in which the scoped_lock object was created, the scoped_lock is destructed and the mutexes are
   released. If several mutexes are given, deadlock avoidance algorithm is used as if by std::lock.

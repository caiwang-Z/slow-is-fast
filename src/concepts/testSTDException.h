...
class VideoPlayerException : public std::exception {
  ...
  const char* what() const noexcept { return _errorMessage; }
...
class A {
  public:
  A() { std::cout << "A constructor calls.\n "; }

  ~A() { std::cout << "A destructor calls.\n "; }
};
class B {
  public:
  B() { std::cout << "B constructor calls.\n "; }

  ~B() { std::cout << "B destructor calls.\n "; }
};
